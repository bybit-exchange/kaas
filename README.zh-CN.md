...
一般的 RAG 系统是把原始文本切块、向量化；KaaS 不一样，它通过一条 4 阶段的 LLM
流水线把内容**编译**成知识。产物是人类可读的 Markdown 文章，而不是一个黑箱向量库。

![KaaS 对比传统 RAG：先编译再检索，而非切块加向量](docs/assets/kaas-vs-rag.zh.svg)

## 编译出来的 Wiki

你拿到的是一棵 Markdown 文章树：左边是分类，中间是渲染后的文章，右边是这一页的
小标题。chat 的回答会点名引用这些文章，答案和它的出处之间只隔一次点击。

![编译好的 wiki：左侧是文章树，右侧是渲染后的 Markdown 文章](docs/assets/screenshot-wiki.zh.png)

## 它是怎么运转的

原始内容经过四个 LLM 阶段：提取概念、实体、决策；分类归入文章；写入或合并
Markdown；更新索引。检索时读的是成篇的文章，不是向量化的碎片 —— LLM 沿着
master-index 走，挑出需要的文章，再基于它们的全文作答。
...
- **产物是文章**：你查询的是成篇的内容，不是按相似度排序的碎片 —— 生成它们的四个阶段见[它是怎么运转的](#它是怎么运转的)
