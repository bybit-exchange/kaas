...
### The queue — 21 items: 18 settled, 3 still needing a ruling
...
call, because each either reclassifies a row or changes what a number means. **V1–V14,
...
blending its two bases into one band, and the one that made P10's roster change additive
— and each row records the decision. **Two items were added after the pass rather
...
a number are ruled**, and none of the 3 still open moves one: V7 left P3 at 8/5, V8 took
...
the survivors. **V1–V14, V19, V20 and V21 are ruled and applied**, so the
...
asserts a replacement instead of a superseded value.
