...
queue stands at **22 rulings** of which **twenty are taken** and V16 needed none, leaving
**one open**. See [the verification pass](#independent-verification-pass-2026-08-15). Note
...
| P4 | 10 | 4 | 6 | **9 of 10** | to confirm — worst case in the set; **V5 ruled 2026-08-16** keeps C8 with a recorded residual, **V15 ruled 2026-08-17** relabels X1 as a drop followed by a supersession over one entity and finds the fixture unable to carry a nested supersession at all, and **V22 ruled 2026-08-17** keeps C9 stale on L667 alone, the other two residues tracing to a passage v2, v3 and v4 assert identically, see [P4](#p4--trade-rollback-trd-four-versions) |
...
### The queue — 22 items: 21 settled, 1 still needing a ruling
...
call, because each either reclassifies a row or changes what a number means. **V1–V15, V17
and V19–V22 are ruled** — the three that moved the totals, the one
...
supersession, the one that held P4-C9 on its one v2-exclusive residue, and the one that
decomposed P10-C5's two days into re-basing — and each row
...
V22 and V17 raised nothing further, so the queue closes at 22. **All three items expected to move
...
match neither of the co-source's columns in any month. **The 1 item still open cannot move
a number either**: V18 is a keep-the-row item recording v1's internal ambiguity under
P4-C3.
...
| P10-C5 / R5 | Median end-to-end delivery cycle ~15 days (13.8 H1, 17.8 H2) → ~17 days (15.3, 19.8) — the pair sets v1's ≤90 population against the **unfiltered** one rather than against the ≤120 one v2 declares (V17) | v1 L24 → v2 L29; basis measured on v2 L51–62 against full L89–100 | not stale, replacement also missing — neither ~15 nor ~17 appears | **V17 ruled 2026-08-17** — row stands, as a contradiction on level and not on trend: both versions state the same statistic over the same span (v1 L144, v2 L14–15) and their H1→H2 changes agree at +29.0% against +29.6%, while the level gap is re-basing — on the same twelve monthly cells the basis change alone accounts for all +2.0 days of H2 and +0.92 of H1's +1.5. Not to be cited as evidence of real slowdown, and the article does not cite it, running its H2 decline on 研发周期 5.0 → 8.5 instead |
...
Two of these were documented when drafted (C7's and C9's). Of the rest, **V5, V10 and V22
are ruled** — V5 kept P4-C8 with its residual recorded, V10 corrected P2's evidence table
and call 2, whose vLLM reading this row had right all along, and V22 kept P4-C9 on the one
residue the ambiguity does not touch — and **V18** is still queued;
