...
in every one**, which is the case's central claim turned from an assertion into a count.
The same parse corrects the two measurements underneath it. The 状态 **cell** differs on
22 of the 273 — always 「🔴 **移除**」 → 「**移除**」 — so a field-equality test reads 22
changed rows where the decision reads none, and the ⚠️ rise is +6 rather than +8, eight
added in the new modules against two lost with the leverage tokens. **What the case gains
is a notation finding**: 🔴 is 24 rows in v1 and 0 in v3, v1 is the only one of the
article's 16 declared sources that marks removals with it, and it survives into the
article at L386 — so the compiled output took its notation from the superseded version
while taking all five figure pairs from v3, and in the other direction v3's own rewritten
P2P wording reaches L402 and L426. Two of the six controls also sit on abridged cells,
...
verification pass and not the confirm pass, which started afterwards and is counted in the
Status block at the head of this file rather than here — a ruling fixes what a row means
where confirmation decides whether it scores.
...
|---|---|---|---|---|
| P3-C1 / R1 | Knowledge caches live in the project, in a gitignored `.claude/knowledge/` → they live at a user-level path shared across projects | v1 L265 「.claude/knowledge/ ← 整个目录 gitignored」 → v2 L152 「**本地克隆路径**：`~/.claude/plugins/cht-knowledge/knowledge/<domain>/`」, L64 | not stale — article states the v2 path (L66), and L66 and L133 name project-level only to reject it | confirmed 2026-08-17 |
| P3-C2 / R2 | Three-level loading with `index.json` permanently resident, ~50 tokens per system and ~1500 for 30 → two-stage loading with nothing resident, ~200 tokens for the summary | v1 L746 「Level 0（常驻）：index.json ~50 tokens/系统」, total at L325 → v2 L951 「两阶段加载在大多数情况下仅消耗 ~200 tokens（Stage 1 摘要）」, L212 | **stale** — six lines, not three: L30 heading, L32 「three loading levels」, L36 table row, L40 「1,500 tokens」, L153 「Level 0, always present」 in the worked example, L214–215 carrying ~1,500 as the base | amended 2026-08-17 — verdict unchanged; the residue was cited at three lines and is six, so a fix that reaches only the architecture section would have scored as complete |
| P3-C3 / R3 | The SessionStart hook generates `index.json` and injects it every session → injection moves to a `UserPromptSubmit` hook on trigger words, and SessionStart only pulls silently | v1 L294 「index.json 从 registry.mjs 自动生成，在每次会话启动时通过 hook 注入 AI 提示词。」 → v2 L214 「**时机**：UserPromptSubmit Hook 检测到触发词时自动注入」, L115 | **stale** — L63 asserts the resident global index, and the every-session half is restated at L36 「Always present in every session」 and L153; L67 also describes v2's pull, so the article carries both mechanisms side by side | amended 2026-08-17 — verdict unchanged; L36 and L153 added as residue, shared with C2 because one article sentence can carry both facets (see open call 7) |
| P3-C4 / R4 | Cached knowledge has a 24-hour validity window and is not re-fetched inside it → freshness comes from a `git pull --rebase` on every session start, failing silently offline | v1 L289 「缓存有效期：24 小时内不重复拉取」 → v2 L251 「对每个知识库执行后台 `git pull --rebase`」, L253 | **stale** — L68 states the 24-hour window as current, one line below L67's v2 mechanism | confirmed 2026-08-17 |
| P3-C5 / R5 | There is no `init` command; installing the plugin completes setup → each knowledge base is installed explicitly with `/cht-knowledge:init <system-id>`, which git-clones it | v1 L399 「**注意：没有 init 命令。** 安装插件即完成所有设置」 → v2 L112, L481/484 | **stale** — L71 「**No init command:**」, and the claim survives in prose at L264 「no further manual steps required」 and L22 「install the plugin once」; `/cht-knowledge:init` is 0 hits in the article | amended 2026-08-17 — verdict unchanged; two prose residues added, so deleting the heading alone does not clear the row |
...
|---|---|---|---|
| P3-D1 | The platform has 100+ repositories | v1 L14 「在中台大团队（100+ 仓库、多个子团队）中」; `100+` and 「子团队」 both 0 hits in v2. **Narrowed on verification** — the original row also claimed "and multiple sub-teams", which v2 restates at L20 「中台有 30+ 微服务，跨团队协作时…」, so that half was not a drop | confirmed 2026-08-17 as a drop — open call 1 answered **no**: repo count and microservice count are different measures, and the article asserts both in one sentence (L16) without incoherence, so there is nothing here for the gate to fire on. The residue is stated as current at L16 |
| P3-D2 | Distillation extracts a minimum knowledge set rather than copying code: 50 Proto methods reduce to 5–10 selected external core interfaces | v1 L561 「**蒸馏不是照搬代码，而是提取 AI 对接时需要的最小知识。**」, L575/578 | confirmed 2026-08-17 — v2 has no minimum-knowledge framing; the article restates v1's example at L101 (50 Proto methods → 5–10) |
| P3-D3 | The transfer system's `Transfer` interface is idempotent on `request_id` with a 3000 ms timeout, `RATE_LIMITED`/`TIMEOUT` retryable and `INSUFFICIENT_BALANCE`/`ACCOUNT_FROZEN` fatal — and, widened by V7, `QueryTransfer` with it (`idempotent: false`, `timeout_ms: 2000`) | v1 L635–638, inside the `interfaces[]` array at L630–646; QueryTransfer at L640–645. Absent from v2 across the whole contract vocabulary — `idempotent`, `idempotent_key`, `timeout_ms`, `retryable_errors`, `fatal_errors`, `RATE_LIMITED`, `TIMEOUT`, `INSUFFICIENT_BALANCE`, `ACCOUNT_FROZEN`, `幂等`, `超时`, `QueryTransfer` all return 0 hits | confirmed 2026-08-17 — **V7 ruled 2026-08-16** — stays a drop, and a **lost** one: the article states the Transfer contract at L119–123 under 「Example `manifest.json` for the transfer system」. QueryTransfer's half has no article residue. Call 2's promotion to C9 declined |
| P3-D4 | The plugin offers `/cht-knowledge:search`, a keyword search over already-cached knowledge | v1 L355/361; v2's command reference (L461–543) has no analogue — `:match` debugs trigger words, it does not search content | confirmed 2026-08-17 — `cht-knowledge:search` is 0 hits in both v2 and the article, so this drop leaves no residue |
| P3-D5 | The first systems to distill are transfer, deposit, withdraw, risk and account | v1 L1113/1116 (L1114 as first drafted is `</lark-td>`) | confirmed 2026-08-17 — 「首批」 is 0 hits in v2, whose roadmap says 3+ systems without naming them; the article names only asset-transfer (L221) |

### Controls (present in both; missing from the article means over-deletion)

| ID | Asserted by both versions | Evidence | Status |
|---|---|---|---|
| P3-K1 | Each system's knowledge lives in its own independent Git repository | v1 L1140–1145 lists one repo per system → v2 L151 「一个领域一个 Git 仓库」 | confirmed 2026-08-17 — both assert it; v2's unit is 领域 where v1's is 系统, which does not change what the control catches. Article carries it at L105 |
| P3-K2 | `cht-context` is a Git submodule bound to a project, unlike `cht-knowledge` which is git-cloned | v1 L160/163 → v2 L72/75, both inside a cht-context-versus-cht-knowledge comparison table | confirmed 2026-08-17 — article carries it at L133, L137 and L231 |
| P3-K3 | Distillation is AI-driven: an agent scans source and generates structured documents | v1 L964 → v2 L97/L109 (L28 as first drafted asserts source→structured docs but not AI authorship) | confirmed 2026-08-17 — article carries it at L21 and L75 |
| P3-K4 | A knowledge base becomes platform-wide by registering in `registry.mjs` and merging an MR | v1 L994/997 → v2 **L742** 「编辑 `plugins/cht-knowledge/lib/registry.mjs`，在 `KNOWLEDGE_REPOS` 数组中添加」 under 「Step 6：注册到 Marketplace」 (L740) **+ L756** 「然后提交 MR … 合并后，所有中台开发人员都可以…」 | amended 2026-08-17 — the v2 side was cited on the MR half only, and `registry.mjs` is v2 L742. Both halves are asserted in both versions, so the control stands; a half-cited control reads like a half-drop when it scores. Article carries it at L129 and L193–206 |
| P3-K5 | Distilled output must be human-reviewed before publication | v1 L972 → v2 L785 | confirmed 2026-08-17 — article carries it at L186 and L262 |
| P3-K6 | On-demand deep loading of full documents costs ~2000–4000 tokens | v1 L796 → v2 L237 | confirmed 2026-08-17 — article carries the same range at L40, L49 and L58 |
...
7. **C2 and C3 are two facets of one architectural shift — answered 2026-08-17: kept separate** (resident index → trigger-based injection), so one article fix would likely satisfy both and that single defect carries double weight. Kept separate because each is independently checkable in prose, and the confirm pass found the overlap to be partial rather than total: L36 and L153 carry both facets, while C2's L32/L40/L214–215 are token-model residue C3 does not touch and C3's L63 is mechanism residue C2 does not. Merging them stays defensible if the score should weight defects rather than sentences — that is a scoring-layer choice, and it does not need the label grain coarsened to make it.
...
The verdict is the earliest assertion against v4's state, which is what the article
is scored on. The Evidence column names **where in the chain each replacement first
appears**, which after the confirm pass is v2 or v3 for nine of the ten rows: v4 is
the version the reader gets, not the version that made the change.

| ID | Earlier assertion → v4's | Evidence | Article today | Status |
|---|---|---|---|---|
| P4-C1 / R1 | Monthly translog gains `change_flow_usd`, `uta_result_topic_name`, `uta_result_offset` → monthly tables get no new fields this round, they go to the real-time table | v1 L1144, **L1354**, L1368 — the three field-name cells; L1363 is the second one's description cell 「topic名字 // 新增的字段」 → **v2**, which both strikes the three rows in the monthly table itself (L1235 `~~**change_flow_usd**~~`, L1445, L1459) and states the rule at L1519; v4 L1458, L1668, L1682 / L1742 are those four lines shifted | **stale** — L454–456 list all three as current additions, and they sit under `#### translog (Sharded)` (L441), which is the monthly table; open actions **L634 and L636**; the correction exists but is misfiled under `uta_liq_trans_log` (L473–478), so the article asserts both | amended 2026-08-17 — verdict unchanged, on three corrections. The replacement lands at **v2**, as V18, V5 and V22 found for C3, C8 and C9, and it is a **cell-level strikethrough** rather than only the prose rule, which is stronger evidence than the row claimed. One v1 cite pointed at a description cell instead of its field name. And the open-action residue is two rows, not one. Recorded: v4 keeps two unstruck bullets asking for the same three fields (L1746–1747) immediately below L1742's denial — residual contrary prose in C7's class, see [call 2](#p4-open-judgement-calls) |
| P4-C2 / R2 | Coin-exchange table is monthly `uta_exchange_record_202604` → a single TiDB table, dual-written with MySQL for a transition | v1 L1433 → **v2** L1526 heading + L1528 「在TiDB中建立单表，不按月分。与mysql双写一段时间。」; v4 L1749/L1751 is that pair shifted 223, the same offset C3 carries | **stale** — L487. **The heading above it is already current**: L485 carries v4's unsuffixed `uta_exchange_record` while L487's body carries v1's `uta_exchange_record_202604` and its monthly claim, so the article splits this pair across a heading and the body under it — C3's shape inverted. 「与mysql双写」 is 0 hits in the article | amended 2026-08-17 — verdict unchanged; the replacement lands at v2, and the article's heading/body split is recorded because it settles [call 9](#p4-open-judgement-calls) from the opposite direction: in this family a heading and its own body drift apart and the compile reads each as an assertion |
| P4-C3 / R3 | §4.1.3 fund-flow table is `uta_liq_trans_log_202605`, monthly, past data deletable → `translog_realtime` in TiDB. Corrected from v4 by V18, as V5 corrected C8 and V22 corrected C9: the heading changes at **v2** | v1 L1761 → **v2** L1888 (v4 L2111 is that heading shifted 223) | **stale** — L469–471, which is its own table section, `#### uta_liq_trans_log (Monthly Partitioned)`, carrying three of v1's four heading clauses | confirmed 2026-08-17 — **V18 ruled 2026-08-17** — row stands, on four v1-exclusive strings rather than one (「按月存储」, 「过往数据可以删除」, 「表结构和translog保持一致」 and the table name, each 1/0/0/0). v1's ambiguity is a heading against its own 774-line section body, which is the earlier version's and so cannot trigger V22's test; what it costs is **R3**, whose only string is v1's own and is control K4's, leaving it scoreable as an attachment and reading not carried — see [call 9](#p4-open-judgement-calls). **Confirmed 2026-08-17**: all four strings re-counted, the 223-line shift re-derived, and call 9 answered **yes** on evidence C2 supplied |
| P4-C4 / R4 | User-behaviour log tables are monthly-sharded `_{yyyyMM}` → unsharded TiDB tables | v1 L2539, L2667, L3653 — the only three `_{yyyyMM}` table names in v1 → **v2** L3034, L3178, L4250 (v4 L3272, L3416, L4488 are those three shifted) | **stale** — L494, L498, L499, **plus L508 and L642**. The article's third table in that section (L500) carries no table name at all, so the residue of v1 L3653 is the `uta_spot_leverage_log_{yyyyMM}` heading at L508 and its open action at L642 — which the draft filed under C5, because the article files both leverage tables together (L502) while v1 files this one inside §4.1.4 用户行为记录表 | amended 2026-08-17 — verdict unchanged; the replacement lands at v2, and the residue is five lines rather than three once the third table is followed to where the article put it |
| P4-C5 / R5 | Existing `uta_leverage_log` will be refactored into monthly shards → migrated from MySQL to TiDB | v1 L3213 「已有uta_leverage_log，改造为按月分表」 (「改造为按月分表」 1/0/0/0) → **v2** L3631/L3633 「uta_leverage_log(TiDB)」 + 「从mysql迁移过来」 (0/1/1/1); v4 L3869, L3871 is that pair shifted | **stale** — L504 heading, L506, action L638. `uta_spot_leverage_log_{yyyyMM}` at L508 is **reassigned to C4**, whose v1 cite L3653 is the table that asserts it; and L506 carries D2's row count in the same sentence, so one article sentence holds two rows' residue | amended 2026-08-17 — verdict unchanged; the replacement lands at v2, and one of the four cited residues belongs to another row |
| P4-C6 / R6 | `set_time` in the user-behaviour log tables is nanoseconds → milliseconds | v1 L2626 (and L2782, L2921, L3179, L3353, L3612, L3754 — seven 「设置时间（纳秒）」 cells) → **v3** L3359, L3405, which v4 carries at the same line numbers. **v2 extends the nanosecond convention instead of changing it**: 7 full-width cells plus 3 half-width 「设置时间(纳秒)」, so v1 and v2 agree here and v3 is the first version to move (13 hits of 「设置时间(ms)」 in v3 and v4, 0 in v1 and v2) | **stale** — L494, and that is the entire residue: the article's only other nanosecond claim is L490's `exec_time_e9`, which v4 still states in nanoseconds (v4 L1976), so it is current rather than stale | amended 2026-08-17 — verdict unchanged; the replacement lands at v3 as V5 and V22 found for C8 and C9, and the residue is confirmed complete at one line |
| P4-C7 / R7 | `uta_auto_add_margin_log` trigger column is `action` → `service_action` | v2 L3954 → **v3** L4192, which v4 carries at the same line number. **`service_action` on its own does not discriminate**: 2 hits in v1 and 3 in v2, all of them the translog tables' own column, so the test needs the `uta_auto_add_margin_log` DDL and not the bare string — the P7-C2 hazard class | **stale** — L514, action L639 | amended 2026-08-17 — verdict unchanged; the replacement lands at v3, and the string hazard is recorded because a grader testing `service_action` document-wide reads this row as already fixed in v1 |
| P4-C8 / R8 | Incident counterparty is the trading system account (系统户), funds settle against it → the in-trading difference account (差额账户). Overdraft is **not** part of the change: 「允许透支」 already stands in v2's cell | v2 L5447, L5453 → **v3** L5697, L5703. Corrected from v4 by V5: `差额账户` first appears in v3, and v4 only shifts these two lines by one with no content change | **stale** — L421–424 | confirmed 2026-08-17 — **V5 ruled 2026-08-16**, row stands with a residual at [call 8](#p4-open-judgement-calls); the confirm pass re-read v2 L5445–5457 against v3 L5695–5707 and the row is replaced cell for cell exactly as V5 recorded, subject and detail 1 changing while the role cell 「作为事故用户的对手方」 stays |
| P4-C9 / R9 | Post-rollback disposal is futures dumped to the order book with options and spot moved to a PM takeover account, pending TR discussion → spot and positions handed to TRW. Corrected from v4 by V22, as V5 corrected C8: `TRW` first appears in v3 | v2 L5455 → **v3** L5705 (v4 L5706 is that line shifted one) | **stale** — on **L667 alone**, the 05-26 action-item row ending 「(discuss with TR)」, a hedge v3 deleted in the same edit that named TRW; TRW appears nowhere. **L425–426 are not evidence**: the passage they trace to stands byte-identical in v2, v3 and v4 | confirmed 2026-08-17 — **V22 ruled 2026-08-17**, row stands on one residue, with the two withdrawn lines recorded as a presentation defect at [call 3](#p4-open-judgement-calls); v2 L5455's hedge and v3 L5705's 「给到TRW处置」 both re-read |
| P4-C10 / R10 | Asset recovery is 60% done → 90% developed, integration testing next week | v3 L5674, the 进展 cell of the 资产追回 row whose subject cell is v3 L5663 → v4 L5675 | not stale — L735 carries v4's value, and `60%` is 0 hits in the article | confirmed 2026-08-17 — the subject cell added so the cite locates the row rather than a bare number, and the absence of v3's value from the article checked rather than assumed |

### Drops (measured, not gating)

| ID | Asserted earlier, absent from v4 | Evidence | Status |
|---|---|---|---|
| P4-D1 | The engine exposes `TradeEngineService` with batch ban/unban RPCs (`BatchBanTrading` / `BatchUnbanTrading`, carrying `user_ids`, `operator`, `reason`, `expire_at`) | v1 L705–737, the four request fields at L717–720; `TradeEngineService` (1/0/0/0) and `BatchBanTrading` (3/0/0/0) are already gone at **v2**, and v4 §3.3 (heading L702, IDL block L705–743) defines no service | amended 2026-08-17 — stays a drop, and a lost one; the drop lands at v2, and the article residue is **five** hits rather than four: L333, L357, L396 and L563 assert the ban/unban role, while L721 lists `[[TradeEngineService]]` under Related Concepts with no predicate attached, so a fix that clears the four role sentences still leaves the entity named |
| P4-D2 | `uta_leverage_log` already holds ~166.6M rows (`AUTO_INCREMENT = 166624305`) | v1 L3236; the number does not occur in v2, v3 or v4 | amended 2026-08-17 — stays a drop, with a string hazard recorded: the article writes it `~166,624,305` (L506), so `grep -F 166624305` scores this residue as absent. Same class as P7's `3,864` |
| P4-D3 | The engine reconciles/tops up to a system account, which can be created in advance | v1 L4776; 对账补齐 and 提前创建 are each 1/0/0/0 across the chain | confirmed 2026-08-17 — both strings re-counted; the residue is article L396, where it sits inside a sentence about v1's own dropped service (D1), which is why X1 reads the two oldest states as asserted side by side |
| P4-D4 | Coin-exchange flows roll back through the normal coin-exchange path | v2 L5442 「3、兑币的流水回滚 （走正常的兑币链路回滚）」; **v3** already ends that cell at item 2 (v3 L5685–5688), and v4 L5686–5689 is the same cell shifted one | amended 2026-08-17 — stays a drop; it lands at v3 rather than v4, and the task's string is 「兑币的流水回滚」 — the form 「兑币流水回滚」 this file used elsewhere is 0 hits in all four versions and has been corrected in the transition table and the X1 write-up |

### Controls

| ID | Asserted by v1 and v4 | Evidence | Status |
|---|---|---|---|
| P4-K1 | The transaction log table is sharded by month and UID trailing digit | v1 L760 → v4 L1074, the same heading verbatim 「### 4.1.1、交易日志表translog_202604_12 （按月份和UID尾号分表）」 | confirmed 2026-08-17 — article carries it at L443. Note what this control does for C1: monthly sharding survives the storage change, which is why C1 is "no new fields this round" and not "the monthly tables are gone" |
| P4-K2 | Users who changed account mode, toggled collateral, changed single/dual position mode, or are institutional borrowers are not auto-rolled-back | v1 L172 → v4 L174, verbatim identical | confirmed 2026-08-17 — article carries all four criteria at L304–308 |
| P4-K3 | `per_user_max_subsidy_usd_amount` defaults to 10000 and `user_rollback_watermark` to 90 | key cells v1 L3995, L4012 → v4 L4848, L4865; default cells v1 L4001, L4018 → v4 L4854, L4871 | amended 2026-08-17 — the row was cited on the default cells alone, which are bare 「10000」 and 「90」 and locate nothing; the key cells are now cited with them. P3-K4's shape: a half-cited control reads like a half-drop when it scores. Article carries both at L530–531, and in prose at L349 and L353 |
| P4-K4 | `translog_realtime` is a single TiDB table, daily partitions, 7-day rolling window | v1 L1763 → v4 L2115 (v4 prefixes the same line with 「(TiDB)」) | confirmed 2026-08-17 — article carries it at L462. This is the line V18's caveat on R3 turns on: it is v1's own, so it cannot separate a carried replacement from a kept control |
| P4-K5 | `mark_price` applies only to USDC perpetuals | v1 L1279 → v4 L1593, verbatim 「标记价格（仅usdc永续）」 | confirmed 2026-08-17 — article carries it at L450 |
| P4-K6 | Volume estimate ~91 million rows / 55GB over 7 days | v1 L2529 → v4 L2896, verbatim | confirmed 2026-08-17 — article carries it at L443, the same sentence that carries K1, so one article line holds two controls |
...
兑币的流水回滚 item at L5442) or material whose value v1 and v2 state identically and only v3
...
The confirm pass of 2026-08-17 answered the one call that was still a question,
**call 9**, at the drafted position, and re-verified calls 1 and 7 against the
sources. Calls 3, 4, 8 and 9 were settled by rulings V22, V15, V5 and V18;
calls 2, 5, 6 and 7 record standing options rather than open asks, and their
defaults hold.

1. **C6 is deliberately scoped to `set_time` in the user-behaviour log tables
   only.** v4 still has `exec_time_e9` in nanoseconds (L1976). Do not widen C6 to "all
   timestamps are milliseconds" — that would be wrong. **Re-verified 2026-08-17**:
   v4 L1976 still reads 「执行时间（纳秒）」, and the article states it at L490, so that
   line is current rather than stale and the article's two nanosecond claims split
   one stale (L494) and one correct. Correction from the same check — the unstruck
   bullets at v4 L1746–1747 this call used to list here are **C1's** residual, not
   C6's: they ask for the three new fields, not for a timestamp unit, and they are
   now recorded on C1.
...
7. **Only C10 comes from v3→v4.** Add the MarginDB cell (v3 L5774 → v4 L5775) if
   ...
   empty in v3, so that is an addition, not a supersession. **Checked 2026-08-17,
   and it would not add gating power**: v3 L5774 「开发中」 → v4 L5775 「联调中」 verifies,
   but the article's L736 「MarginDB: already deployed to mainnet; integration testing
   ongoing」 carries v4's state, so the row would read **not stale** exactly as C10
   does. Adding it doubles the v3→v4 evidence and leaves the staleness column at
   9 of 10 either way — which is the real finding, because it means this chain's
   last step is one A1 gets right twice, not once.
...
9. **C3 is a heading-only claim, and R3 has no string of its own — V18 ruled 2026-08-17.**
   ...
   **Answered 2026-08-17 with the confirm pass: yes, a heading asserts, and the row
   keeps its drafted position.** Two things decided it, neither available when V18 ruled.
   First, **C2 is the same defect mirrored**, which makes it a property of this document
   family rather than a one-off: there the article's heading is *current* (L485
   `uta_exchange_record`) while the body under it is *stale* (L487's monthly name), where
   in C3 v1's heading is the stale side and its own section body specifies the
   replacement. A heading and the body beneath it drift apart in both directions here,
   and the compile reads each as an assertion — so treating one as mere decoration would
   mis-score whichever side happens to be wrong. Second, the drift is **editorial rather
   than semantic**: v2 rewrites both headings in the same edit and at the same offset
   (v2 L1526 → v4 L1749 and v2 L1888 → v4 L2111 are both shifted 223), which is what an
   author does to a claim they are changing, not to a label they consider inert.
   Declining the call would have taken P4 to 9C and 8 of 9 stale, and set totals to 43C
   with the headline at 26 of 38 — so the drafted position is also the one that moves
   nothing.
...
**plus L721** as a bare Related-Concepts link, added by the confirm pass),
P4-D2 `AUTO_INCREMENT ~166,624,305` (L506 — written with commas, so the raw
digits find nothing), P4-D3 pre-created reconciliation
...
count was corrected from three. **All six controls are present, and the confirm
pass cites where**: K1 and K6 at L443, K2 at L304–308, K3 at L530–531, K4 at
L462, K5 at L450.
...
endpoint rows the name and the method are identical in every one, the 状态 decision is
identical in every one, and no row reverses a call. The 17 备注 differences are
abridgements. Two measurements first recorded here were too strong and are corrected in
the confirm pass below: the 状态 *cell* differs on 22 rows, by the 🔴 glyph v3 stops
using, and five of the 17 abridgements are not trailing-clause deletions. Neither
changes the shape — it is additive scope plus a recount, the easiest supersession
shape there is. Report P5 as a confounded, low-difficulty pass and do not aggregate
its 0-of-5 with a case like P7 without that caveat.

**Measured in the confirm pass, 2026-08-17.** Parsing both files into endpoint rows —
lark tables and v1's 22 markdown pipe rows alike — gives **278 rows in v1 and 299 in
v3, sharing 273 paths**, with 5 v1-only (all `spot-lever-token`) and 26 v3-only, which
is call 2's arithmetic reproduced. It also reconciles the two bases this case is stated
on: `/v5/private` and `/v5/public/` occur in both files outside any table, so the 280
and 301 totals are 278 + 2 and 299 + 2, and the same two-endpoint allowance turns 256
and 277 tabled retentions into the published 保留 258 and 279. Three arms of the
comparison then read:

- **状态**: 22 differences, every one of them 「🔴 **移除**」 → 「**移除**」. Both
  versions carry exactly 22 removal rows, so dropping the Leverage Token module did not
  touch the count — its five rows were 保留 and ⚠️, not 移除.
- **⚠️**: 29 cells in v1 against 35 in v3. The composition is 8 added in v3's three new
  modules **minus the 2 lost with the leverage tokens**, so the rise is +6 and not the
  +8 call 2 recorded — and +6 is exactly what the two summary tables publish (~25 →
  ~31), each of them 4 below its own cell count.
- **备注**: 17 differences, matching call 3's list row for row.

**The article's notation blends the two versions even though its figures do not.** 🔴 is
a v1 fingerprint: 24 rows in v1, 0 in v3, and v1 is the only one of the article's 16
declared sources that marks removals with it — the sole other source using the glyph at
all is the design doc, twice for Critical severity and once for a 强推 recommendation. It reaches the article at L386,
「Removed (🔴)」. In the other direction, the `p2p/order/finish` note is the one
abridgement that is a rewrite rather than a deletion, 「释放 crypto 给买家，需人工确认法币
到账，AI 无法判断」 → 「释放 crypto 给买家，AI 无法判断法币到账」, and the article carries
**v3's** fused form twice (L402, L426). So the glyph is a seventh way to tell the two
files apart — carrying no direction, unlike families 2–6, but unlike them also surviving
...
|---|---|---|---|---|
| P5-C1 / R1 | The inventory covers 8 skill modules → 11, the new three being Copy Trading, Strategy Orders and Trading Bot | v1 L13 → v3 L12, sections L5649, L5753, L5856 | not stale — L32 states 11 modules | confirmed 2026-08-17 — the count has a mechanism under it, which is what makes the row testable: v1's section headings name **8 distinct `*.md` module files** and v3's name 11, the additions being `copy-trading.md`, `strategy.md` and `trading-bot.md`, exactly the three sections cited. 「8 modules」 and 「8 个 modules」 are 0 hits in the article |
| P5-C2 / R2 | Total audited API count 280 → 301 | v1 L5438, L5441 → v3 L6341, L6344 | not stale — L141; `280` occurs nowhere | amended 2026-08-17 — verdict unchanged, both cites verify and `280` is 0 hits; the article side was cited one line short of itself, stating 301 at **four** lines (L32, L141, L383, L564), so a fix reaching L141 alone would leave three standing |
| P5-C3 / R3 | Retained endpoints 258 → 279 | v1 L5446, L5449 → v3 L6349, L6352 | not stale — L142; no `258` | amended 2026-08-17 — verdict unchanged, cites verify and `258` is 0 hits; 279 is stated at L32 as well as at the cited L142. Both figures are their document's own arithmetic on the same allowance — 256 and 277 tabled retentions plus the two untabled WebSocket endpoints — and each equals its own GET/POST split |
| P5-C4 / R4 | Retained split ~165 GET / ~93 POST → ~169 GET / ~110 POST | v1 L5454–5465 → v3 L6357–6368 | not stale — L142 | amended 2026-08-17 — verdict unchanged; the split is stated at L32 as well as at the cited L142, and the article drops the tildes (`169`, `110`). This is the one row whose absence test is safe in either form: `~165`, `~93`, bare `165` and bare `93` are all 0 hits in the article |
| P5-C5 / R5 | Endpoints needing Mainnet confirmation ~25 → ~31 | v1 L5470, L5473 → v3 L6373, L6376 | not stale — L144, L404 | amended 2026-08-17 — verdict unchanged, and two hazards attach. The article states ~31 at **four** lines (L144, L162, L385, L404), two of them uncited. And the superseded side must be tested on 「~25」: bare `25` hits nine article lines (2025, 25%, and so on). Note both versions understate their own ⚠️ cells by exactly 4 — v1 tables 29 and publishes ~25, v3 tables 35 and publishes ~31 — so the row scores on the published pair and the article inherits v3's undercount |

### Drops (measured, not gating)

| ID | Asserted by v1, absent from v3 | Evidence | Status |
|---|---|---|---|
| P5-D1 | The skill inventories a Leverage Token module (`/v5/spot-lever-token/*`, 5 endpoints under `spot.md`), Purchase and Redeem needing Mainnet confirmation | v1 L1241, endpoints L1273–1349; verified absent (`spot-lever-token`, `杠杆代币`: 0 hits in v3) | confirmed 2026-08-17 — the five endpoints are Info, Reference, Purchase, Redeem and Order Record, and the confirmation half is corroborated by arithmetic rather than by reading: **Purchase (v1 L1315) and Redeem (v1 L1335) are the module's only two ⚠️ cells**, which is what makes v1's 29 ⚠️ cells become v3's 35 once the 8 new ones are added. See call 1, and **V9 ruled 2026-08-16**: the article does state this material (L413, L567) but it is **not counted as a lost drop**, two declared never-superseded sources asserting it independently |
| P5-D2 | When cancelling all orders, take care not to cancel strategic standing orders | v1 L584; v3 L583 reduces the note to 「Mainnet 需确认」 | confirmed 2026-08-17 — both cells verify verbatim and 「策略性挂单」 is 1 hit in v1, 0 in v3. **V9 ruled 2026-08-16**: **not** a lost drop, 「注意不要误取消策略性挂单」 having no residue in the article |
| P5-D3 | Batch order placement must display all orders before confirmation | v1 L604; v3 L603 reduced | confirmed 2026-08-17 — v3 L603 is the bare 「Mainnet 需确认」 and 「展示全部订单」 is 1 hit in v1, 0 in v3. **V9 ruled 2026-08-16**: **not** a lost drop, and the near-miss is the article's generic confirmation card (L320, L580), which is not 「展示全部订单再确认」 |
| P5-D4 | The BybitPay Payout endpoint is essentially an alternative withdrawal channel | v1 L4744 「向任意 UID 打款，本质是另一种提现通道」 → v3 L4889 「向任意 UID 打款」. Test on 「另一种提现通道」, 0 hits in v3 — **not** on 「向任意 UID 打款」, which is v3 L4889's own surviving cell and is asserted in the same abridged form by the declared, never-superseded `…保留接口清单按场景.md` L3550 | confirmed 2026-08-17 — both cells verify verbatim and 「另一种提现通道」 is 1 hit in v1, 0 in v3. **V9 ruled 2026-08-16**, a **lost** drop: article L427 states it as "alternative withdrawal channel (sends funds to any UID)". The residue is not v1-exclusive corpus-wide — the undeclared, unstaged hardening plan carries the clause at L96, with a 商户 the article does not have — so this row sits on D6's footing rather than D5's, see [V16](#v16--resolved-by-investigation-no-ruling-needed) |
| P5-D5 | The stated *reason* for removing institutional-loan UID bind/unbind — that it affects other users' accounts — is dropped; v3 keeps the removal with no rationale | v1 L4007 「机构贷款绑定/解绑 UID，影响他人账户」 → v3 L4044 「机构贷款绑定/解绑 UID」; `影响他人账户` 0 hits in v3 — **not** `影响他人`, which hits v3 L2958 「冻结子账户影响他人使用」. **Reworded on verification** — as first drafted the row folded in the surviving removal, so a grader applying the absence test to the row as written finds it present in v3 and mis-scores it | confirmed 2026-08-17 — the cells and both string tests verify: 「影响他人账户」 1 hit in v1 and 0 in v3, 「影响他人」 2 and 1, the surviving v3 hit being L2958. **V9 ruled 2026-08-16**, a **lost** drop: article L402 states it as "institutional loan UID binding (affects other accounts)". The residue is a v1 fingerprint, 「影响他人账户」 being a single corpus hit, and the hardening plan drops the clause on its own line for this endpoint (plan L502) |
| P5-D6 | Universal Transfer's stated risk — that funds can be moved to a sub-account under someone else's control — is dropped; v3 keeps the endpoint, its 移除 status and the bare 跨 UID 转账 classification | v1 L2730 「跨 UID 转账，资金可转到他人控制的子账户」 → v3 L2652 「跨 UID 转账」. Test on the full clause 「资金可转到他人控制的子账户」 or on 「他人控制」, both 0 hits in v3 — **not** on 「他人」, which hits v3 L2958 「冻结子账户影响他人使用」, a row identical in both versions | confirmed 2026-08-17 — the cells and both string tests verify (「资金可转到他人控制的子账户」 and 「他人控制」 are each 1 hit in v1, 0 in v3). One cite-grain note: v1 states this endpoint as a **markdown pipe row** (L2730, one of its 22) where v3 states it as a lark table, so the superseded side is a whole row and the replacement side a single cell — the two arms cannot be cited at the same grain here. **V8 ruled 2026-08-16**, promoted from [call 3](#p5-open-judgement-calls), and a **lost** drop: article L394 states it as 「Funds can reach accounts controlled by others」. Note the residue is not v1-exclusive corpus-wide (the undeclared hardening plan carries the clause at L116), but that document reaches neither the article nor the fixture — see [V16](#v16--resolved-by-investigation-no-ruling-needed) |
...
| ID | Asserted by both | Evidence | Status |
|---|---|---|---|
| P5-K1 | 22 endpoints removed on security grounds, split 12 P0 and 10 P1 | v1 L5481, L5489, L5497 → v3 L6384, L6392, L6400 | amended 2026-08-17 — the three number cells verify on both sides, and the control holds on content and **not on bytes**: its row label is 「🔴 **移除**」 in v1 (L5478) and 「**移除**（安全）」 in v3 (L6381). It also holds independently of the summary tables — both versions carry exactly 22 rows whose 状态 is 移除. Article L143 states 22 with the 12 P0 / 10 P1 split |
| P5-K2 | Move Position and its history endpoint are removed because they move positions across UIDs | v1 L970 → v3 L969 | amended 2026-08-17 — the control holds, and its cell is one of the 17 v3 abridged: 「跨 UID 移动仓位，等于转移持仓价值」 → 「跨 UID 移动仓位」, so a presence test on v1's cell reads over-deletion where the control survives. 「转移持仓价值」 is 0 hits in v3 and the article states it at L397 as 「Cross-UID position value transfer」 — the article carries the clause v3 dropped, on the very line that carries the control. Its history endpoint is abridged the same way (v1 L990 → v3 L989) |
| P5-K3 | Withdraw is removed because it sends funds to external addresses | v1 L2723 → v3 L2612 | confirmed 2026-08-17 — the two cells are byte-identical, 「提现到外部地址，资金直接离开平台」; the article carries both halves at L393 |
| P5-K4 | Modify Master API Key is removed because the AI could grant itself Withdraw permission | v1 L3055 → v3 L2998 | amended 2026-08-17 — K2's shape again: the cells are identical up to a parenthetical v3 drops, 「AI 可给自己加 Withdraw 权限（权限提升）」 → 「AI 可给自己加 Withdraw 权限」, with 「权限提升」 0 hits in v3 and the article stating it at L396 as 「Privilege escalation」 and restating the capability at L399. So two of P5's six controls sit on abridged cells |
| P5-K5 | Upgrade to UTA is removed as irreversible | v1 L2039 → v3 L1930 | confirmed 2026-08-17 — byte-identical, 「不可逆操作，升级后无法回退」; the article states both clauses at L425 and lists the endpoint at L402 |
| P5-K6 | Leverage above 20x triggers an extra warning beyond Mainnet confirmation | v1 L802 → v3 L801 | confirmed 2026-08-17 — byte-identical including the 「Mainnet 需确认；」 prefix the warning is additional to; the article states it at L407 |

### P5 open judgement calls

All five were **answered on 2026-08-17 with the confirm pass**, none of them moving a
verdict: call 1 keeps its drafted position, calls 2 and 3 are corrected on measurements
rather than on judgements, and calls 4 and 5 keep their lists with the provenance now
cited. **P5 leaves nothing with Captain.**
...
1. **P5-D1: drop or contradiction?** v3 never says leverage tokens are out of
   ...
   in error. **Answered at the drafted position in the confirm pass**, which also
   corroborated the row's confirmation half from the other side: the module's only two
   ⚠️ cells are Purchase (v1 L1315) and Redeem (v1 L1335), and dropping them is what
   makes v1's 29 ⚠️ cells reach v3's 35 after the 8 additions.
2. **No status decision changed anywhere — but the status cell did.** All **273**
   pairable endpoint rows match on name and on method, with zero differences in either
   field, and no row reverses a 保留 / ⚠️ / 移除 call. (251 as first drafted was wrong:
   v1 has 278 REST endpoint rows and v3 299, sharing 273 paths, since 278 − 5
   leverage-token rows = 273.) **The confirm pass corrects two measurements inside this
   call**, neither of them touching the conclusion. The 状态 *cell* differs on **22 of
   the 273**, in every case 「🔴 **移除**」 → 「**移除**」, so an equality test on the
   field reads 22 changed rows where a test on the decision reads none — the count of
   removal rows is 22 in both versions. And the ⚠️ rise is **+6, not +8**: eight are
   added in the three new modules and **two are lost with the leverage tokens**
   (29 cells → 35), which is why both summary tables publish a 6-endpoint rise. So this
   revision offers no reversed-decision contradiction — it is additive scope plus a
   recount. Verified mechanically: 280 distinct `/v5/...` paths in v1, 301 in v3, 5
   dropped (all `spot-lever-token`), 26 added, and 280 − 5 + 26 = 301 — a path basis
   that exceeds the row basis by the two untabled WebSocket endpoints, `/v5/private`
   and `/v5/public/`, which both documents also count inside their 保留 figures.
3. **Note abridgement below article altitude — settled by V8, ruled 2026-08-16.**
   ...
   **The 17 were reproduced mechanically in the confirm pass**, row for row against this
   list, and the reproduction corrects the case blurb rather than the disposition:
   **five of the 17 are not trailing-clause deletions.** `user/delete-api` loses its
   *leading* clause (「删除主 API Key，锁死用户自己的访问」 → 「锁死用户自己的访问」),
   `move-history` and `query-universal-transfer-list` lose a leading token (both
   「⟨sibling⟩ 配套查询，一并移除」 → 「配套查询，一并移除」), `crypto-loan-fixed/borrow`
   loses a *middle* clause (「固定期限借贷，产生利息负债，有清算风险」 → 「固定期限借贷，有清算
   风险」), and `p2p/order/finish` is a **rewrite**: 「释放 crypto 给买家，需人工确认法币到账，
   AI 无法判断」 → 「释放 crypto 给买家，AI 无法判断法币到账」. The last one is the only cell
   in the case where v3 says something in words v1 does not use, and it is v3's form
   that reaches the article (L402, L426) — the mirror of the 🔴 residue in call 4.
4. **Left out**: the 🔴 glyph on removal rows (presentation), and endpoint index
   ...
   grader must not match on endpoint numbers). **Still left out after the confirm pass,
   but it is not inert.** The glyph is not a per-row flourish: it is 24 rows in v1 and
   **0 in v3**, a notation v3 abandons wholesale, and v1 is the only one of the
   article's 16 declared sources that marks removals with it — the design doc is the
   sole other user of the glyph, twice for Critical severity and once for a 强推
   recommendation. It reaches the
   article at L386, 「Removed (🔴)」. It asserts no proposition, so it scores nothing and
   stays out; what it is evidence of is that the article's *notation* came from the
   superseded version while all five of its figure pairs came from v3.
5. **Unrelated stale content, not this chain's**: L189 「274+ API endpoints across
   ...
   false-positive on them. **Both provenances are now cited**: the 274 is
   `raw/docs/2026-03-11-bybit-ai-trading-skill-能力清单.md` L14 (「零安装，274 个 API
   全覆盖」) and the 7 modules is `raw/docs/2026-03-10-bybit-ai-trading-agent-设计方案.md`
   L362 (「将 SKILL.md 拆分为路由主文件 + 7 个功能模块」), both declared sources and
   neither superseded. Note this makes **three** module counts on three different bases
   in one article — 7 SKILL.md files, and the chain's 8 → 11 inventory modules — so
   「modules」 is the one noun in this case a grader must never match a number against.
