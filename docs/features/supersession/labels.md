...
by rulings since — V19's ruling raised V20 and V20's raised V21 — so the queue stands at
**21 rulings** of which **seventeen are taken** and V16 needed none, leaving **three open**. See [the verification pass](#independent-verification-pass-2026-08-15). Note
...
| P4 | 10 | 4 | 6 | **9 of 10** | to confirm — worst case in the set; **V5 ruled 2026-08-16** keeps C8 with a recorded residual, **V15 ruled 2026-08-17** relabels X1 as a drop followed by a supersession over one entity and finds the fixture unable to carry a nested supersession at all, and **V22 is open** on whether C9 keeps its stale verdict when two of its three article residues also stand in v4, see [P4](#p4--trade-rollback-trd-four-versions) |
...
### The queue — 22 items: 19 settled, 3 still needing a ruling
...
call, because each either reclassifies a row or changes what a number means. **V1–V15,
V19, V20 and V21 are ruled** — the three that moved the totals, the one
...
blending its two bases into one band, the one that made P10's roster change additive, and
the one that took P4's chained-supersession entry down to a drop followed by a single
supersession — and each row records the decision. **Three items were added after the pass
rather than found by it**: V19's ruling raised **V20**, V20's raised **V21** and V15's
raised **V22**, each because settling the item exposed the defect under it. Ruling V21
raised nothing further, so the queue stands at 22. **All three items expected to move
a number are ruled**, and of the 3 still open only V22 could move one, narrowly: V7 left P3 at 8/5, V8 took
...
entry would have left NG3 with nothing rather than with less. **V22 is the one open item
that could still move a total**, and only just: it asks whether P4-C9 keeps its stale
verdict now that two of its three article residues are shown to be present in v4 as well,
which would take P4 from 9 of 10 to 8 of 10 and the published rate from 27 of 39 to 26 of
39 — but its third residue, L667's 「(discuss with TR)」, is v2-exclusive in the chain and
would have to fall too, so the likelier outcome is a caveat in C5's shape.
...
| **V22** | P4-C9 | P4 → contradictions table | Raised by ruling V15. **Of C9's three article residues only one discriminates a version.** L425 「a **TWAP market-selling mechanism** similar to the insurance pool」 traces to v4 L5832 「采用类似保险池twap甩卖机制进行盘口甩卖」 and L426 「either held to expiry or transferred to a PM takeover account」 to v4 L5833 「期权仓位如何处置： 1.等交割 2.通过移仓给到PM接管户」 — pairings v2's superseded cell does not carry, since it puts 现货 with 期权 and offers no 等交割 — and both v4 lines stand unchanged from v2 (L5561–5562), so under V19's ruled criterion the newest source speaking to them is v4 itself. Only L667's 「(discuss with TR)」 is v2-exclusive in the chain (`TR讨论` 1 hit in v2, 0 in v1, v3 and v4), as is the 「交易系统户」 the article heads L421 with. A second defect sits in the same two lines and is not staleness: the article states v4's 待决策 options as settled characteristics, dropping the hedge the source attaches to them | Decide whether C9 stays stale on a single v2-exclusive residue, and record the lost 待决策 hedge separately — the shape P10's C2 already carries, presentation rather than supersession. Note the source-side ambiguity is already documented in [call 3](#p4-open-judgement-calls); what is new is that the *article's wording* sides with the pending-decision block | If C9 falls, P4 goes 9 of 10 → **8 of 10** and the published rate 27 of 39 → **26 of 39 (67%)**. If it stands, it stands with C5's kind of caveat and the hedge-dropping becomes its own finding. **The only open item that could move a total** |
...
the survivors. **V1–V15, V19, V20 and V21 are ruled and applied**, so the
...
the next number could actually move.
...
| P4-C9 / R9 | Post-rollback disposal is futures dumped to the order book with options and spot moved to a PM takeover account, pending TR discussion → spot and positions handed to TRW | v2 L5455 → v4 L5706 | **stale** — L425–426, plus open item L667; TRW appears nowhere | to confirm — **V22 raised 2026-08-17** by ruling V15: of that evidence only L667 is version-discriminating, see [call 3](#p4-open-judgement-calls) |
...
are 0 hits in v2–v4); L421's heading 「System Account (交易系统户)」 and L423's settlement /
overdraft sentence are v2's superseded ones (交易系统户 is 1 hit in v2 and 0 in v1, v3 and
v4); L667 「System account: incident-period fund settlement, derivatives position
...
chain). Against that, 差额 and TRW are **0 hits in the article**. So the trail A2 has to
...
3. **C9 is the least certain, and V15's ruling made it less so — queue item V22,
   raised 2026-08-17.** v4's 待决策 section still lists TWAP order-book selling and the
   PM-account transfer as options (L5832 「采用类似保险池twap甩卖机制进行盘口甩卖」, L5833
   「期权仓位如何处置： 1.等交割 2.通过移仓给到PM接管户」), while the module table says TRW.
   Scored on the module table because the other block is explicitly marked "pending
   decision". What was not recorded is that **the article's wording sides with the
   pending-decision block rather than with v2's superseded cell**: L425 「a **TWAP
   market-selling mechanism** similar to the insurance pool」 matches v4 L5832's 保险池 /
   twap pairing, which v2's cell does not carry, and L426 「either held to expiry or
   transferred to a PM takeover account」 matches v4 L5833's 等交割 option, which v2's
   cell also lacks — v2 instead puts 现货 with 期权 and offers no 等交割. Both lines stand
   unchanged in v2 (L5561–5562) and in v4, so **neither discriminates a version**, and
   C9's version-discriminating residue is L667's 「(discuss with TR)」 alone. A second
   defect sits in the same place and is not staleness: the article states these as
   settled characteristics, dropping the 待决策 hedge the source attaches to them.
...
Only one of these was documented when drafted (C7's). Of the rest, **V5 and V10 are
ruled** — V5 kept P4-C8 with its residual recorded, V10 corrected P2's evidence table and
call 2, whose vLLM reading this row had right all along — and **V18** is still queued;
P6's changes no scored row and is recorded rather than queued.
