...
def test_update_people_stubs_skips_files_outside_the_wiki_dir(tmp_path):
    ...
    store.write_article("wiki/a.md", "[[Lucas Wan]] spoke")
    outside = tmp_path / "outside.md"
    outside.write_text("[[Lucas Wan]] also mentioned here")
    ...
    people.update_people_stubs(fake_store, [{"canonical": "Lucas Wan", "aliases": ["Lucas Wan"]}])

    stub = (store.wiki_dir / "people" / "lucas-wan.md").read_text()
